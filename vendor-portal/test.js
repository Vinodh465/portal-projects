...
const query = (path) => new Promise((resolve, reject) => {
  http.get('http://localhost:4300/sap/opu/odata/sap/ZMM_VENDOR_SRV_093_SRV/' + path, {
    headers: {
...
const endpoints = [
  ...
  'PoSet?$format=json',
  'GrSet?$format=json',
  'InvoiceSet?$format=json',
  'PaymentAgingSet?$format=json',
  'CreditSet?$format=json',
  'ProfileSet(\'0001000000\')?$format=json'
];
