...
const sets = ['PoSet', 'GrSet'];

Promise.all(sets.map(s => query(`${s}?$filter=substringof('${belnr}', PurDoc) or substringof('${belnr}', PoNumber) or substringof('${belnr}', MatDoc)`))).then(results => {
  results.forEach(res => {
    console.log(`\n--- ${res.path} ---`);
    try {
